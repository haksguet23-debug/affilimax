...
if DEMO_MODE:
    print("[STRIPE] Stripe non configure - les API de paiement sont indisponibles (STRIPE_SECRET_KEY manquante)")

# NOTE sur le mode LIVE : le check de demo-data-in-live (partenaires avec
# emails @example.com alors que STRIPE_SECRET_KEY est sk_live_...) est
# fait par mode_reel_guard.py et expose via l'endpoint /api/go-live/status.
# On n'ajoute PAS de garde-fou ici pour eviter un circulaire import
# (load_partners est definit juste apres).

# ==================== GESTION DES PARTENAIRES ====================

# Verrou pour proteger partners.json contre les race conditions.
# add_commission_to_partner et update_partner font un cycle
# load->modify->save qui doit etre atomique, sinon deux requetes
# paralleles ecrasent les donnees l'une de l'autre.
_PARTNERS_LOCK = threading.RLock()
...
def init_partners():
    """Cree un fichier partenaires vide (100% reel, pas de donnees demo)."""
    now = datetime.utcnow().isoformat() + "Z"
    partners = {
        "timestamp": now,
        "partenaires": []
    }
    # init_partners est appele uniquement depuis load_partners, qui tient deja le lock.
    # On utilise save_partners pour l'ecriture atomique ; avec RLock la re-acquisition
    # est sans danger (meme thread).
    save_partners(partners)
    return partners

def save_partners(data):
    """Sauvegarde les donnees partenaires (thread-safe + ecriture atomique).
    
    Ecrit d'abord dans un fichier temporaire puis le renomme de maniere atomique
    (os.replace = atomique sur POSIX et Windows). Cela empeche toute lecture d'un
    fichier partiellement ecrit en cas de crash ou de race.
    """
    with _PARTNERS_LOCK:
        tmp_path = PARTNERS_FILE + ".tmp"
        try:
...
def update_partner(partner_id, updates):
    """Met à jour un partenaire (thread-safe)."""
    with _PARTNERS_LOCK:
        data = load_partners()
        for p in data["partenaires"]:
            if p["id"] == partner_id:
...
def add_commission_to_partner(partner_id, amount_eur, source="commission"):
    """Ajoute une commission au solde d'un partenaire (thread-safe)."""
    with _PARTNERS_LOCK:
        data = load_partners()
        for p in data["partenaires"]:
            if p["id"] == partner_id:
...
def create_connect_account(partner_id, email, nom, country=None, currency=None, business_type=None):
    ...
    """
    if not stripe:
        return {"success": False, "error": "Stripe indisponible - verifiez STRIPE_SECRET_KEY et le module stripe"}
...
def regenerate_onboarding_link(stripe_account_id, partner_id):
    ...
    """
    if not stripe:
        return {"success": False, "error": "Stripe indisponible"}
...
def get_partner_stripe_status(stripe_account_id):
    ...
    """
    if not stripe:
        return {"exists": False, "mode": "error", "message": "Stripe indisponible"}
...
def create_payout_to_partner(partner_id, amount_eur, idempotency_key=None):
    ...
    """
    if not stripe:
        return {"success": False, "error": "Stripe indisponible - impossible de creer un payout"}
