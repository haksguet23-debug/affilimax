...
# === INJECTION ENV VARS (Stripe LIVE + Admin) ===
# ⚠️ PRODUCTION (Render.com) : Les cles DOIVENT venir de Render Dashboard > Environment.
#    Ne JAMAIS commiter de cles LIVE dans le code source.
# ⚠️ DEV LOCAL : La cle ci-dessous est un fallback temporaire pour le dev.
#    SUPPRIME-LA avant de commit.
# ⚠️ STRIPE_SECRET_KEY DOIT etre defini dans l'environnement (pas en dur dans le code).
# La cle ci-dessous est un placeholder de dev qui sera ignore par Stripe.
# Pour le mode LIVE : export STRIPE_SECRET_KEY=sk_live_...
...
if not os.environ.get("ADMIN_PASSWORD"):
    os.environ["ADMIN_PASSWORD"] = "08hx9C_oJBwJHyVSWtzgOwaIgdqdZf0a"
if not os.environ.get("AFFILMAX_REQUIRE_LIVE"):
    os.environ["AFFILMAX_REQUIRE_LIVE"] = "0"  # Mode DEV : pas de blocage Stripe/Partner
if not os.environ.get("PARTNER_SECRET_KEY"):
    os.environ["PARTNER_SECRET_KEY"] = "affilimax-dev-secret-key-change-in-production-32chars"
...
# Chaque client a sa propre queue.Queue. publish_event() pousse a tous.
# Aucun polling cote front : les dashboards recoivent les events en live.
...
def publish_event(event_type, data):
    """Pousse un event temps-reel a tous les clients EventSource connectes."""
    payload = {"event": event_type, "data": data, "ts": datetime.utcnow().isoformat() + "Z"}
...
if os.environ.get("AFFILMAX_REQUIRE_LIVE", "0") == "1" and not ADMIN_AUTH_REQUIRED:
    raise RuntimeError(
...
def get_products_tuples():
    ...
    Robuste face aux JSON incomplets (cles manquantes)."""
    produits = load_products()
    return [(
        p.get("nom", "Inconnu"),
        p.get("plateforme", "Inconnue"),
        float(p.get("prix", 0.0)),
        float(p.get("commission_pct", 0.0)) / 100
    ) for p in produits if p.get("actif")]
...
def record_click(product_name=None, platform=None, source=None):
    ...
    if source and source in SOURCES:
        ...
        final_source = "referencement_direct"
    ...
    if camp_idx is None:
        if product_name and platform:
            ...
            # Pas de produit specifie : on refuse l'enregistrement aleatoire
            return None
...
def record_conversion(product_name=None, platform=None, commission_override=None, price_override=None, partner_id=None):
    ...
    for camp in data["top_campagnes"]:
        if camp["nom"] == final_product_name:
            ...
            camp["progression"] = min(100, camp["progression"] + 2)
            found_camp = True
            break
    if not found_camp:
    ...
    # Credit la balance du partenaire (si partner_id fourni) - CRITIQUE pour monétisation
    if partner_id and commission > 0:
        try:
            new_balance = stripe_config.add_commission_to_partner(partner_id, commission, source="conversion")
            if isinstance(new_balance, (int, float)):
                print(f"  [COMMISSION] {partner_id} +{commission:.2f} EUR (new_balance={new_balance:.2f} EUR)")
            else:
                print(f"  [COMMISSION] {partner_id} +{commission:.2f} EUR (partner_not_found_or_failed)")
            try:
                from notifications import notify_commission
                partner = stripe_config.get_partner(partner_id)
                if partner:
                    notify_commission(partner, commission, final_product_name)
            except Exception as ne:
                print(f"  [COMMISSION] notif skipped: {ne}")
        except Exception as e:
            print(f"  [COMMISSION] ERREUR credit {partner_id}: {e}")

    # Balance du partenaire : utilise DIRECTEMENT la valeur retournee par
    # add_commission_to_partner (deja ecrit dans partners.json par stripe_config).
    # Evite une 2e lecture du fichier et le risque de race avec un autre thread.
    # new_balance est defini uniquement si partner_id fourni + commission > 0.
    partner_balance = None
    if 'new_balance' in locals() and isinstance(new_balance, (int, float)):
        partner_balance = float(new_balance)

    publish_event("conversion", {
        "produit": final_product_name,
        "plateforme": final_platform,
...
class AffilimaxHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ADMIN GATE : /admin.html, /payouts.html et /admin/* demandent une auth (Basic)
        if path in ("/admin.html", "/payouts.html") or path.startswith("/admin/"):
        ...
        # Idem pour /api/partners et /api/partners/<id> : le contenu contient
        # emails, IBAN Stripe Connect, transactions, payouts, preferences.
        if (path.startswith("/api/stripe/") and path not in ("/api/stripe/webhook",)) \
                or path.startswith("/api/partners"):
            if not _require_admin_auth(self):
                return

        # Diagnostic mode reel : revele l'etat prod (mode Stripe, longueur
        # du mot de passe admin, partenaires reels) -> protege par admin.
        if path == "/api/go-live/status":
            if not _require_admin_auth(self):
                return
        ...
        # API: Activite recente (20 derniers events) - feed live pour dashboard
        if path == "/api/stats/recent":
            try:
                data = load_data()
                # isinstance strict : si legacy JSON a un dict/str au lieu d'une
                # list, on retourne [] (fail-safe) plutot que de transformer en
                # list() qui cacherait la corruption (ex: dict -> [keys]).
                activite = data.get("activite_recente")
                if isinstance(activite, list):
                    recent = activite[:20]
                else:
                    recent = []
                self.serve_json({"recent": recent, "count": len(recent), "ts": data.get("timestamp", "")})
            except Exception as e:
                self.serve_json({"recent": [], "error": str(e)})
            return

        # SSE STREAM : /api/stream - bus temps-reel pour dashboards
        if path == "/api/stream":
            try:
                ...
                with _SSE_LOCK:
                    _SSE_CLIENTS.add(q)
                try:
                    init_payload = {"event": "connected", "data": {"clients": len(_SSE_CLIENTS)}, "ts": datetime.utcnow().isoformat() + "Z"}
                    self.wfile.write(("data: " + json.dumps(init_payload, ensure_ascii=False) + "\n\n").encode("utf-8"))
                    self.wfile.flush()
                    hb = 0
                    while True:
                        try:
                            msg = q.get(timeout=15)
                            self.wfile.write(msg.encode("utf-8") if isinstance(msg, str) else msg)
                            self.wfile.flush()
                            hb = 0
                        except _queue.Empty:
                    ...
                    # Nettoyage garanti meme sur BrokenPipeError (client ferme)
                    with _SSE_LOCK:
                        _SSE_CLIENTS.discard(q)
            except Exception as e:
                print(f"  [SSE] /api/stream error: {e}", file=sys.stderr)
            return
        ...
        # API: Diagnostic mode reel (checklist pre-prod)
        # Renvoie un JSON par brique avec ok:true/false + raison
        # Permet a un dashboard ou un cron externe de verifier l'etat
        if path == "/api/go-live/status":
            try:
                p_data = stripe_config.load_partners()
                partners = (p_data or {}).get("partenaires", []) if p_data else []
                try:
                    n_cfg = json.loads((BASE_DIR / "notifications_config.json").read_text(encoding="utf-8"))
                except Exception:
                    n_cfg = {"telegram": {}, "slack": {}}

                import re as _re
                demo_p = [p for p in partners
                          if isinstance(p.get("email"), str)
                          and _re.search(r"@example\.|@test\.|@demo\.",
                                         p.get("email", "").lower())]

                sk = os.environ.get("STRIPE_SECRET_KEY", "")
                wh = os.environ.get("STRIPE_WEBHOOK_SECRET", "")
                flag = os.environ.get("AFFILMAX_REQUIRE_LIVE", "0")
                ...
                checks = {
                    "stripe": {
                        ...
                        "mode": "live" if sk.startswith("sk_live_") else (
                            "test" if sk.startswith("sk_test_") else "demo"),
                        "masked_key": (sk[:11] + "..." + sk[-4:]) if sk else None,
                    },
                    "webhook": {
                        "ok": wh.startswith("whsec_"),
                        "signing": "HMAC" if wh.startswith("whsec_") else "none",
                    },
                    "admin_auth": {
                        "ok": bool(au and ap and len(ap) >= 12),
                        "admin_user_set": bool(au),
                        "admin_password_len": len(ap) if ap else 0,
                    },
                    "require_live": {
                        "ok": flag == "1",
                        "value": flag,
                    ...
                    "partners": {
                        "ok": len(demo_p) == 0 or not sk.startswith("sk_live_"),
                        "total": len(partners),
                        "demo_count": len(demo_p),
                        "real_count": len(partners) - len(demo_p),
                    },
                    "telegram": {
                        "ok": bool(n_cfg.get("telegram", {}).get("enabled")
                                   and n_cfg.get("telegram", {}).get("bot_token")
                                   and n_cfg.get("telegram", {}).get("chat_id")),
                    },
                    "slack": {
                        "ok": bool(n_cfg.get("slack", {}).get("enabled")
                                   and n_cfg.get("slack", {}).get("webhook_url")),
                    },
                    "resend": {
                        "ok": rk.startswith("re_"),
                    },
                }
                total = len(checks)
                ready = sum(1 for c in checks.values() if c.get("ok"))
                payload = {
                    "checks": checks,
                    "score": {"ready": ready, "total": total,
                              "percent": round(ready * 100 / total, 1)},
                    "overall_ready": ready == total,
                }
        ...
        # Page admin preferences : explicit handler (subdir served via SimpleHTTPRequestHandler)
        if path == "/admin/preferences.html":
            self.path = "/admin/preferences.html"
            return super().do_GET()
    ...
    def do_POST(self):
        ...
        if path == "/api/click":
            ...
            if data:
                ...
                self.serve_json({"status": "error", "message": "Produit requis pour enregistrer un clic"})
            return
        ...
        if path == "/api/conversion":
            ...
            partner_id = payload.get("partner_id") or payload.get("ref")
        ...
        if path == "/api/partner/preferences":
            token = payload.get("token", "")
            from partner_auth import verify_session, update_partner_preferences
            partner = verify_session(token)
            if not partner:
                self.serve_json({"success": False, "error": "Session invalide ou expiree"})
            else:
                ...
                if not isinstance(prefs, dict):
                    self.serve_json({"success": False, "error": "preferences doit etre un dict"})
                else:
                    result = update_partner_preferences(partner["id"], prefs)
                    self.serve_json(result)
            return
            ...
            if stripe_config.stripe and stripe_config.STRIPE_WEBHOOK_SECRET:
                try:
                    ...
                    if event_type == "payout.paid":
                        # Payout plateforme (Stripe -> compte bancaire Affilimax)
                        po_id = event_obj.get("id")
                        po_amount = event_obj.get("amount", 0) / 100
                        po_status = event_obj.get("status", "paid")
                        print(f"[STRIPE] Payout plateforme paye: {po_id} | {po_amount:.2f} EUR | status={po_status}")
                        # Mettre a jour les payouts partenaires correspondants
                        try:
                            data = stripe_config.load_partners()
                            updated = 0
                            for p in data.get("partenaires", []):
                                for po in p.get("payouts", []):
                                    # Match par transfer_id ou idempotency_key dans metadata
                                    if po.get("transfer_id") == po_id or po.get("id") == po_id:
                                        po["statut"] = "completed"
                                        po["date_fin"] = datetime.utcnow().isoformat() + "Z"
                                        updated += 1
                                        print(f"[STRIPE]   -> payout {po['id']} marque complete pour {p['nom']}")
                            if updated:
                                stripe_config.save_partners(data)
                        except Exception as e:
                            print(f"[STRIPE] Erreur MAJ payout: {e}")
                    elif event_type == "payout.failed":
                        po_id = event_obj.get("id")
                        print(f"[STRIPE] Payout plateforme echoue: {po_id}")
                        try:
                            data = stripe_config.load_partners()
                            for p in data.get("partenaires", []):
                                for po in p.get("payouts", []):
                                    if po.get("transfer_id") == po_id or po.get("id") == po_id:
                                        po["statut"] = "failed"
                                        po["failure_reason"] = event_obj.get("failure_message", "Inconnu")
                                        print(f"[STRIPE]   -> payout {po['id']} marque echoue pour {p['nom']}")
                            stripe_config.save_partners(data)
                        except Exception as e:
                            print(f"[STRIPE] Erreur MAJ payout failed: {e}")
                        ...
                        acct_id = event_obj.get("account")
                        print(f"[STRIPE] capability.updated: {cap}={status} (account={acct_id})")
                        # Si transfers devient active, mettre a jour le partenaire
                        if status == "active" and "transfers" in str(cap):
                            try:
                                data = stripe_config.load_partners()
                                for p in data.get("partenaires", []):
                                    if p.get("stripe_account_id") == acct_id:
                                        if not p.get("onboarded"):
                                            ...
                                            p["stripe_onboarding_date"] = datetime.utcnow().isoformat() + "Z"
                                            p["derniere_activite"] = datetime.utcnow().isoformat() + "Z"
                                            print(f"[STRIPE]   -> partenaire {p['id']} onboarded via capability.updated")
                                            stripe_config.save_partners(data)
                                        break
                            except Exception as e:
                                print(f"[STRIPE] Erreur capability.updated: {e}")
                    elif event_type in ("transfer.created", "transfer.paid"):
                        # Virement Connect vers un partenaire
                        transfer_id = event_obj.get("id")
                        transfer_amount = event_obj.get("amount", 0) / 100
                        dest_acct = event_obj.get("destination")
                        print(f"[STRIPE] {event_type}: {transfer_id} | {transfer_amount:.2f} EUR | dest={dest_acct}")
                        try:
                            data = stripe_config.load_partners()
                            for p in data.get("partenaires", []):
                                if p.get("stripe_account_id") == dest_acct:
                                    # Chercher le payout correspondant
                                    for po in p.get("payouts", []):
                                        if po.get("transfer_id") == transfer_id:
                                            po["statut"] = "paid" if event_type == "transfer.paid" else "processing"
                                            po["stripe_transfer_status"] = event_type
                                            if event_type == "transfer.paid":
                                                po["date_fin"] = datetime.utcnow().isoformat() + "Z"
                                            print(f"[STRIPE]   -> payout {po['id']} -> {po['statut']} pour {p['nom']}")
                                            break
                                    stripe_config.save_partners(data)
                                    break
                        except Exception as e:
                            print(f"[STRIPE] Erreur transfer webhook: {e}")
                    else:
                ...
                # n'importe quel etat via ce webhook. Le webhook est fail-closed.
                _fail_closed = (
                ...
                if _fail_closed:
                    ...
                    return
                print(f"[STRIPE] Webhook REJETE : STRIPE_WEBHOOK_SECRET manquant (mode DEMO/fallback desactive)")
                self.send_response(503)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({
                    "status": "error",
                    "error": "Webhook signature verification required. Set STRIPE_WEBHOOK_SECRET."
                }, ensure_ascii=False).encode("utf-8"))
                return
    ...
    def _handle_redirect_impl(self, slug):
        ...
        # Essayer par correspondance de slug (correspondance exacte)
        if not target:
            for p in produits:
        ...
        # Essayer par correspondance de nom (fallback)
        if not target:
...
def main():
    ...
    # Trafic automatique SUPPRIME - seuls les clics/conversions REELS sont acceptes
    print("   [TRAFIC] 100%% REEL - Aucune simulation automatique")
    print()
