...
# Verrouillage thread-safe pour les acc s concurrents au calendrier
...
def generate_content_for_product(product, prenom="Ami"):
    ...
    content_type = random.choices(
        CONTENT_TYPES, weights=CONTENT_WEIGHTS, k=1)[0]
    ...
    if content_type == "tweet_promo":
        ...
        return {
            "type": "email",
            "objet": tpl["objet"].format(**context),
            "content": tpl["corps"].format(**context)}
...
def save_calendar(data):
    ...
    clean = json.loads(
        json.dumps(
            data,
            default=lambda x: list(x) if isinstance(
                x,
                set) else x))
    with CALENDAR_LOCK:
...
def add_to_history(entry):
 ...
 # Stats
    ...
    cal["stats"]["by_platform"][platform] = cal["stats"]["by_platform"].get(
        platform, 0) + 1

    today = datetime.utcnow().strftime("%Y-%m-%d")
    cal["stats"]["by_day"][today] = cal["stats"]["by_day"].get(today, 0) + 1
    cal["stats"]["last_24h"] = sum(
        1 for h in cal["history"] if (
...
class PromoAutomator:
    ...
    def _generate_and_publish(self):
        ...
        # Notification asynchrone (thread daemon, ne bloque pas la publication
...
def print_stats():
    ...
    print(
        f" Produits postes: {stats['products_posted']}/{stats['total_products']}")
