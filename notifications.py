...
def _get_partner_pref(partner_nom, key):
    """Lit la preference du partenaire depuis partners.json (lecture a la volee).

    Retourne True par defaut si :
    - le partenaire n est pas trouve
    - le fichier partners.json est inaccessible
    - la cle de preference est absente

    Retourne la valeur explicite (True/False) sinon.
    Usage : if not _get_partner_pref(partner_nom, "email_payout"): return {}
    """
    try:
        with open(BASE_DIR / "partners.json", "r", encoding="utf-8") as f:
            data = json.load(f)
        for p in data.get("partenaires", []):
            if p.get("nom") == partner_nom:
                return p.get("preferences", {}).get(key, True)
    except Exception:
        pass
    return True
...
def notify_payout(partner_nom, montant, statut="completed", payout_id=None, mode="demo"):
    ...
    Args:
        ...
        mode: demo ou live
    """
    if not _get_partner_pref(partner_nom, "email_payout"): return {}
    config = load_config()
...
def notify_commission(partner_nom, montant, source="vente", total_solde=None):
    ...
    if not _get_partner_pref(partner_nom, "email_commission"): return {}
    config = load_config()
    timestamp = datetime.now().strftime("%d/%m/%Y %H:%M")
...
def notify_threshold(partner_nom, montant, seuil):
    ...
    if not _get_partner_pref(partner_nom, "email_threshold"): return {}
    config = load_config()
    timestamp = datetime.now().strftime("%d/%m/%Y %H:%M")
