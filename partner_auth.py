...
def _sanitize_partner(partner):
    ...
    return {
        ...
        "preferences": {**default_prefs, **(partner.get("preferences") or {})},
    }


# ==================== PREFERENCES (NEW) ====================

# Liste des cles de preferences valides (defense en profondeur contre
# l'injection de cles arbitraires via API)
VALID_PREF_KEYS = ("email_payout", "email_threshold", "email_commission")


def update_partner_preferences(partner_id, prefs_dict):
    """Met a jour les preferences d'un partenaire (subset autorise).

    Args:
        partner_id: ID interne du partenaire
        prefs_dict: dict des preferences a mettre a jour
                   (clés : email_payout, email_threshold, email_commission)

    Returns:
        dict avec success: True/False et le partenaire sanitize, ou error
    """
    from stripe_config import load_partners, save_partners
    data = load_partners()
    target = None
    for p in data["partenaires"]:
        if p["id"] == partner_id:
    ...
    # Merge defensive : on n'ecrit QUE les cles valides + booleens
    current = target.get("preferences") or {}
    for k in VALID_PREF_KEYS:
        if k in prefs_dict:
            v = prefs_dict[k]
            if isinstance(v, bool):
                current[k] = v
    target["preferences"] = current
    target["derniere_activite"] = datetime.utcnow().isoformat() + "Z"
    save_partners(data)
    return {"success": True, "preferences": _sanitize_partner(target)["preferences"]}
...
def get_partner_stats(partner_id):
    ...
    return {
        ...
        "preferences": _sanitize_partner(partner)["preferences"],
    }
